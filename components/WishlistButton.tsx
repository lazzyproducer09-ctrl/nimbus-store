...
export function WishlistButton({
  ...
  if (variant === "labeled") {
    return (
      ...
      >
        <HeartIcon className={`h-5 w-5 ${active ? "fill-red-500 text-red-500" : ""}`} />
        {active ? "Saved" : "Save"}
      </button>
    );
  }
  ...
  return (
    <button
      ...
      className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-ink shadow-sm transition-transform hover:scale-110"
    >
      <HeartIcon className={`h-4 w-4 ${active ? "fill-red-500 text-red-500" : ""}`} />
    </button>
  );
}
